...
| DGX Spark | PuppyPi (real, over seam) | v1.8 single-shot: inference/application → seam dispatch (`run_inference.py --seam`) | raw_zenoh seam plugin (`puppypi_bridge` body) | Validated | Level 3 real-body path | Real-dog run 2026-09-03: RPi5 frame → DGX `vllm_openai` (~1 s) → `gesture_commander` → `--seam raw_zenoh` → real PuppyPi, `path=bridge` (STOP ~515 ms, TROT ~1013 ms). Motion gated (`--allow-motion` + bounded lease + auto-STOP), outcome→exit code, `--action-target` selects the body. `scripts/run_inference.py`, `docs/v1.8-live-body.md` |
| DGX Spark | PuppyPi (real, over seam) | v1.8 live loop: observe→infer→confirm→dispatch (`run_live.py`) | raw_zenoh seam plugin (`puppypi_bridge` body) | Validated | Level 3 live control | Real-dog run 2026-09-03: RPi5 JPEG snapshot source → DGX live loop → raw_zenoh → real PuppyPi. Real vLLM: room → `stop` → dog stop (edge-triggered). Forced `TROT` (mock): confirmed on the 2nd frame → dog trots (`path=bridge` ~1019 ms) → bounded run → **automatic STOP**. Multi-observation confirmation + stall watchdog + fail-safe STOP. `scripts/run_live.py`, `docs/v1.8-live-body.md` |
| Any brain | RPi 5 + hobby servo (real) | general **ROS-free** actuator: `gpio_servo` adapter → GPIO (gpiozero), same seam + `{action, params}` | raw_zenoh seam plugin (`gpio_servo` body) | Validated | Level 3 live control | Real-hardware run 2026-09-03: DGX brain `run_live` (mock TROT) → raw_zenoh → RPi 5 real `gpio_servo` body (`AngularServo` GPIO18). Confirmed on the 2nd frame → `trot` dispatched; body returned `completed` with servo detail `{sweep_deg: 40, hz: 2}` (the servo swept), edge-triggered hold, bounded run → **automatic STOP** (centered). Servo motion independently confirmed via a `gpiozero` preflight; `GpioServoAdapter` also has 10 dry-run unit tests. Demonstrates a general, ROS-free body — same seam/contract, only `ROBOT_ADAPTER` changes. `control_daemon/adapters.py` (`GpioServoAdapter`, also `LedAdapter` — software-only), `tests/test_gpio_servo_adapter.py`, `docs/runbooks/v1.8-quick-demo.md` |
