...
| Any brain | RPi 5 + hobby servo (real) | general **ROS-free** actuator: `gpio_servo` adapter → GPIO (gpiozero), same seam + `{action, params}` | raw_zenoh seam plugin (`gpio_servo` body) | Partial | Level 3 (adapter + hardware bring-up) | 2026-09-03: `GpioServoAdapter` implemented (`stop`/`home` center, `trot` sweeps in a background thread, `move` steers by `yaw`) with 10 dry-run unit tests; loads and `dispatch("trot")` → `completed` on the RPi 5; the servo itself confirmed moving via a standalone `gpiozero` `AngularServo` (GPIO18) preflight. **Full brain→seam→real-servo live loop is documented but not yet run end-to-end.** Demonstrates a general, ROS-free body — same seam/contract, only `ROBOT_ADAPTER` changes. `control_daemon/adapters.py` (`GpioServoAdapter`, also `LedAdapter` — software-only), `tests/test_gpio_servo_adapter.py`, `docs/runbooks/intern-demo-day.md` |
